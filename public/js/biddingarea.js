...
mode.addEventListener("click", () => {
    if (state == 0) {
        ...
        document.querySelector(".header").style.border = "0px 0px 1px 0px solid white";
        ...
        document.querySelector(".header").style.border = "0px 0px 1px 0px solid black";
         document.querySelectorAll("h1").forEach((ele) => {
...
function displayData(data) {
  ...
  document.getElementById('pickupLocation').textContent = `${data.fromAddress.city}, ${data.fromAddress.state}`;
  document.getElementById('pickupDate').textContent = formatDate(data.pickupDate);
  document.getElementById('destination').textContent = `${data.toAddress.city}, ${data.toAddress.state}`;
  document.getElementById('estimatedDelivery').textContent = calculateEstimatedDelivery(data.pickupDate);

  document.getElementById('weight').textContent = data.dimensions.weight || 'N/A';
  document.getElementById('dimensions').textContent = `${data.dimensions.length} x ${data.dimensions.width} x ${data.dimensions.height}` || 'N/A';
  ...
  const sortedBids = data.Bids.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).reverse();
...
function calculateEstimatedDelivery(pickupDateStr) {
  ...
  const estimatedDeliveryDate = new Date(pickupDate.getTime() + 2 * 24 * 60 * 60 * 1000);
